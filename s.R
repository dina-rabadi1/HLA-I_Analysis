...
if (num_samples >= 2 && num_samples <= 4) {
  ...
  for (set_name in names(intersection_peptides)) {
    ...
    Peptide Length`[which.max(length_dist$n)]
    ...
    if (num_samples >= 2 && num_samples <= 4) {
      ...
      for (set_name in names(intersection_peptides)) {
        ...
        Peptide Length`[which.max(avg_sharing_by_length$avg_samples)]
        ...
        # Add the plots
        # 1. Peptide Distribution by Sample Count
        dashboard_html <- paste0(dashboard_html, wrap_in_div(p1_interactive, "Peptide Distribution by Sample Count"))
        
        # 2. Unique vs Shared Peptides
        dashboard_html <- paste0(dashboard_html, wrap_in_div(p2, "Unique vs Shared Peptides"))
        
        # Close first container and open second
        dashboard_html <- paste0(dashboard_html, '\n</div>\n<div class="dashboard-container">\n')
        
        # 3. Peptide Length Distribution
        dashboard_html <- paste0(dashboard_html, wrap_in_div(p3_interactive, "Peptide Length Distribution"))
        
        # 4. Average Sharing by Peptide Length
        dashboard_html <- paste0(dashboard_html, wrap_in_div(p4_interactive, "Average Sharing by Peptide Length"))
        
        # Close second container and open third
        dashboard_html <- paste0(dashboard_html, '\n</div>\n<div class="dashboard-container">\n')
        
        # 5. Gene Heatmap
        dashboard_html <- paste0(dashboard_html, wrap_in_div(p5, "Peptides by Gene Across Samples"))
        
        # 6. Sample Intersections (if available)
        if (exists("p6")) {
          dashboard_html <- paste0(dashboard_html, wrap_in_div(p6, "Sample Intersections"))
        }
...
# 
# # Load required libraries
# required_packages <- c("tidyverse", "UpSetR", "VennDiagram", "RColorBrewer",
#                        "plotly", "htmlwidgets", "openxlsx")
# 
# # Install and load necessary packages
# for (pkg in required_packages) {
#   if (!require(pkg, character.only = TRUE)) {
...
# viz_dir <- paste0(output_dir, "/visualizations")
# dir.create(viz_dir, showWarnings = FALSE)
# 
# # Create output directory for Excel reports
# excel_dir <- paste0(output_dir, "/excel_reports")
...
#   most_common_length_pct <- round(max(length_dist$percentage))
#   best_sharing_length <- avg_sharing_by_length# R script for interactive peptide sample distribution visualization
#   # This builds on your working script to add interactive plots and Excel reports
# 
#   # Setting directory
#   setwd("~/Documents/Github/HLA-I_Analysis/")
# 
...
#     }
# 
#     cat("\nTo view the interactive dashboard, open this file in a web browser:\n")
#     cat(paste0(viz_dir, "/peptide_sharing_dashboard.html"), "\n")