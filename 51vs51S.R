...
# Create PNG version
png(file.path(data_path, "junction_peptide_presence_heatmap.png"), width = 800, height = 600, res = 100)
pheatmap(
  presence_matrix,
  main = "Presence of DNAJB1_PRKACA Junction Peptides Across Samples",
  color = c("white", "darkblue"),
...
# Create PNG version
png(file.path(data_path, "junction_peptide_intensity_heatmap.png"), width = 800, height = 600, res = 100)
pheatmap(
  log_intensity_matrix,
  main = "Intensity of DNAJB1_PRKACA Junction Peptides Across Samples (log10)",
  color = colorRampPalette(c("white", "steelblue", "darkblue"))(100),
...
# 3. Bar plot comparing 51 vs 51S for each peptide with log scale
if (all(c("51", "51S") %in% sample_ids)) {
  comparison_plot_data <- peptide_matrix_data %>%
    filter(Sample_ID %in% c("51", "51S")) %>%
    mutate(
      Sample_Type = ifelse(Sample_ID == "51S", "51S (Spiked 10ng)", "51 (Natural)"),
      # Add small value to prevent log(0) errors
      plot_intensity = total_intensity + 1
    )
  
  # Create PDF version with log scale
  pdf(file.path(data_path, "51_vs_51S_comparison_log.pdf"), width = 10, height = 8)
  comparison_plot_log <- ggplot(comparison_plot_data, aes(x = Peptide, y = plot_intensity, fill = Sample_Type)) +
    ...
    labs(
      title = "Comparison of Junction Peptide Detection: 51 vs 51S (Log Scale)",
      subtitle = "51S sample has 10ng of each peptide spiked in",
      x = "Peptide Sequence",
      y = "Total MS1 Intensity (log10 scale)",
      fill = "Sample"
    ) +
    # Add text labels showing exact intensity values
    geom_text(aes(label = scales::scientific(total_intensity)), 
              position = position_dodge(width = 0.9),
              vjust = -0.5, 
              size = 3)
  
  print(comparison_plot_log)
  dev.off()
  
  # Create PNG version with log scale
  png(file.path(data_path, "51_vs_51S_comparison_log.png"), width = 800, height = 600, res = 100)
  print(comparison_plot_log)
  dev.off()
  
  # Also create the original linear scale version in both formats
  ...
  # PNG version of linear scale
  png(file.path(data_path, "51_vs_51S_comparison.png"), width = 800, height = 600, res = 100)
  print(comparison_plot)
  dev.off()
}
