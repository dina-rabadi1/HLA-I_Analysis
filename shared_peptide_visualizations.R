...
# To fix the error "argument of length 0" in the dashboard creation section,
# replace the entire dashboard creation section (section 7) with this code:

################################################################################
# 7. Create a comprehensive HTML dashboard with all visualizations
################################################################################

# Directly save the individual visualizations first to ensure they exist
# This step is important - we need to make sure all plots are saved as separate files
cat("Saving individual visualizations...\n")

# We've already saved these above, but we'll be explicit here
htmlwidgets::saveWidget(p1_interactive, paste0(viz_dir, "/interactive_sample_count_distribution.html"), selfcontained = TRUE)
htmlwidgets::saveWidget(p2, paste0(viz_dir, "/interactive_unique_vs_shared.html"), selfcontained = TRUE)
htmlwidgets::saveWidget(p3_interactive, paste0(viz_dir, "/interactive_length_distribution.html"), selfcontained = TRUE)
htmlwidgets::saveWidget(p4_interactive, paste0(viz_dir, "/interactive_avg_sharing_by_length.html"), selfcontained = TRUE)
htmlwidgets::saveWidget(p5, paste0(viz_dir, "/interactive_gene_heatmap.html"), selfcontained = TRUE)

if (exists("p6")) {
  htmlwidgets::saveWidget(p6, paste0(viz_dir, "/interactive_sample_intersections.html"), selfcontained = TRUE)
}

# Create HTML header with CSS styling - with double percent signs to escape % in CSS
html_header <- '
<!DOCTYPE html>
<html>
<head>
...
# Define file paths for each visualization
plot_files <- list(
  p1 = "interactive_sample_count_distribution.html",
  p2 = "interactive_unique_vs_shared.html",
  p3 = "interactive_length_distribution.html",
  p4 = "interactive_avg_sharing_by_length.html",
  p5 = "interactive_gene_heatmap.html"
)

if (num_samples >= 2 && num_samples <= 4) {
  plot_files$p6 <- "interactive_sample_intersections.html"
}

# Create plot titles
plot_titles <- list(
  p1 = "Peptide Distribution by Sample Count",
  p2 = "Unique vs Shared Peptides",
  p3 = "Peptide Length Distribution", 
  p4 = "Average Sharing by Peptide Length",
  p5 = "Peptides by Gene Across Samples"
)

if (num_samples >= 2 && num_samples <= 4) {
  plot_titles$p6 <- "Sample Intersections"
}

# Open first dashboard container div
dashboard_html <- paste0(dashboard_html, '\n<div class="dashboard-container">\n')

# Add the first two plots
dashboard_html <- paste0(dashboard_html, create_plot_iframe(plot_files$p1, plot_titles$p1))
dashboard_html <- paste0(dashboard_html, create_plot_iframe(plot_files$p2, plot_titles$p2))

# Close first container and open second
dashboard_html <- paste0(dashboard_html, '\n</div>\n<div class="dashboard-container">\n')

# Add the second two plots
dashboard_html <- paste0(dashboard_html, create_plot_iframe(plot_files$p3, plot_titles$p3))
dashboard_html <- paste0(dashboard_html, create_plot_iframe(plot_files$p4, plot_titles$p4))

# Close second container and open third
dashboard_html <- paste0(dashboard_html, '\n</div>\n<div class="dashboard-container">\n')

# Add the fifth plot
dashboard_html <- paste0(dashboard_html, create_plot_iframe(plot_files$p5, plot_titles$p5))

# Add the sixth plot if it exists
if (!is.null(plot_files$p6)) {
  dashboard_html <- paste0(dashboard_html, create_plot_iframe(plot_files$p6, plot_titles$p6))
}
