...
# Add transcriptome processing if available
if (!is.null(config$transcriptome_path) && file.exists(config$transcriptome_path)) {
  cat("\n## 3a. Processing transcriptome data...\n")
  
  # For multi-sample, we need to extract sample pattern from config
  transcriptome_data <- process_transcriptome_data(
    config$transcriptome_path,
    gene_col = "symbol",
    sample_pattern = config$sample_pattern  # Use the sample pattern to auto-detect columns
  )
} else {
  transcriptome_data <- NULL
}

# Update multi-omics integration section
if (!is.null(transcriptome_data) || !is.null(lfq_data) || !is.null(tmt_data)) {
  cat("\n## Adding multi-omics integration...\n")
  ...
  multi_omics_data <- multi_sample_analysis
}
...
# Create sample-specific visualizations for each sample
if (length(sample_ids) > 1) {
  for (sample_id in sample_ids) {
    # Check if there are peptides specific to this sample
    sample_specific_count <- sum(multi_omics_data[[paste0("total_intensity_", sample_id)]] > 0 &
                                   rowSums(multi_omics_data[, paste0("total_intensity_", setdiff(sample_ids, sample_id))]) == 0)
    
    if (sample_specific_count > 0) {
      # Filter to just sample-specific peptides
      sample_specific_data <- multi_omics_data %>%
        filter(!!sym(paste0("total_intensity_", sample_id)) > 0) %>%
        filter_at(vars(starts_with("total_intensity_") & !matches(sample_id)), all_vars(. == 0))
      
      # Create the visualization
      sample_specific_plot <- create_tumor_specific_plot(
        data = sample_specific_data,
        tumor_specific_col = "sharing_category",  # For multi-sample this would be different
        intensity_col = paste0("total_intensity_", sample_id),
        tumor_value = "Private",  # For multi-sample, "Private" indicates sample-specific
        label_col = "Peptide",
        title = paste0("Peptides Exclusive to ", sample_id)
      )
      save_plot(sample_specific_plot, file.path(viz_dir, paste0(sample_id, "_specific_peptides")))
    }
  }
}
